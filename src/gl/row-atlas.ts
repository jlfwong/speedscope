...
export class Sky_RowAtlas<K> {
  private texture: Graphics.Texture
  private renderTarget: Graphics.RenderTarget
  private rowCache: LRUCache<K, number>
  private clearLineBatch: RectangleBatch

  constructor(private gl: Graphics.Context, private canvasContext: CanvasContext) {
    this.texture = gl.createTexture(
      Graphics.TextureFormat.LINEAR_CLAMP,
      Math.min(canvasContext.getMaxTextureSize(), 4096),
      Math.min(canvasContext.getMaxTextureSize(), 4096),
    )
